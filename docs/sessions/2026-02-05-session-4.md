...
## Session 4e — Glassmorphic student card + UX improvements (same day, continued)
...
- Card shadow gets a subtle Gold tint (`rgba(212,165,116,0.25)`) if Pro — the only visual difference at the card level besides the badge.
- Dark-mode variants specified (Gold brightens to `#FFD89B`, card bg shifts to `rgba(34,47,69,0.85)`).
- Photo upload: bottom sheet (mobile) / modal (desktop) → camera or library picker → client-side circular crop → WebP upload to Supabase Storage → `users.avatar_url` update. Falls back to initials-in-circle if no photo.
- Skeleton state mirrors card shape (circle + lines) while profile data loads — uses shadcn `<Skeleton>` + `motion-safe:animate-pulse`.
...
- Transition timing scale locked: 100 ms (tap), 150 ms (hover), 200 ms (route/fade), 250 ms (sheet). Only `transform` + `opacity` — never `width`/`height`.
- Touch targets: `+ Add photo` link and Pro badge CTA get 44 × 44 px hit areas even though they're visually small.
...
| Selfie separate from Dow ID photo | Dow ID goes through an approval flow (admin verifies). A profile selfie is casual and instant — different purpose, different upload path. |
...
- `docs/decisions/profile-card-ux.md` — new, LOCKED
- `docs/decisions/ui-page-structure.md` — §3.1 greeting spec added, §10 Profile wireframe replaced with glassmorphic card
- `docs/decisions/mobile-web-ui.md` — sidebar wireframe updated with avatar mini-card, sources updated

### State of the project

- Days 1–4 complete. Decision docs: `rag-architecture.md`, `model-selection.md`, `maps-platform.md`, `education-tab.md`, `mobile-web-ui.md`, `ui-page-structure.md`, `profile-card-ux.md`.
- Every screen is specced. The glassmorphic card component + avatar upload + all UX conventions are ready to build in Phase 2 Days 10–11.
- Days 5–9 remain: Voice/STT, AI routing, Mobile delivery, Viva orchestration, Sign-off.

---

---

## Session 4f — Deep audit + todo rewrite + Windsurf handoff plan (same day, continued)
...
|---|---|---|
| Dow Credits top-up | Easypaisa / JazzCash → manual verify 5–10 min | `credits-payment.md` |
| Pro upgrade flow | Credits deduct PKR 3 000 / yr, annual | merged into `credits-payment.md` |
| DowEats ops | Item-first menu, 6-digit code, gate delivery, 12–1:30 peak | `doweats-ops.md` |
| Marketplace withdrawal | Manual, PKR 500 min, 0% fees, 2–5 days | `marketplace-ops.md` |
| Viva scoring | 3 modes, 50-point scale, per-dimension weights | `viva-scoring.md` |
| L&F contact | Full number + WhatsApp, trust model, 30-day archive | Already in `ui-page-structure.md` §7 — fine |

### Conflicts and stale docs found

| Issue | Where | Severity |
|---|---|---|
| **Rate limit conflict** | `FINAL_LOCKED_DECISIONS.md` says 20 soft / 50 hard. `rag-architecture.md` + CLAUDE.md say 2 soft / 4 hard. | 🔴 Must resolve Day 9. 2/4 is almost certainly correct — the 20/50 predates the cost model. |
| **Nav structure stale** | `5_UXUI_GUIDELINES.md` §2–3 still shows old 5-tab nav (Timetable / Attendance / Learning / Community / Account). | 🟡 Update before Day 10. |
| **Maps section stale** | `00_DISCOVERY_RESOLVED.md` says "Google Maps SDK" | 🟡 Update before Day 10. Superseded by MapLibre. |
| **`FINAL_LOCKED_DECISIONS.md` is a legacy flat file** | 340 lines of Q&A format. No pointers to the new decision docs. | 🟡 Rewrite as an index on Day 9. |

### Windsurf handoff docs identified

Eight docs that don't exist yet but Windsurf needs to build without ambiguity. Tracked in the new "Windsurf Handoff" section of `todo-current.md`. The most critical is `windsurf-rules.md` — the `.windsurf/rules` always-on context file. Write on Day 9 after all decisions are locked.

### Files touched

- `docs/todo-current.md` — full rewrite. Expanded to cover: all locked decisions with summaries, new product/operational decision items, stale doc reconciliation checklist, Phase 2 build items (expanded with glassmorphic card + sidebar avatar + admin queue), parallel workstream, and the Windsurf Handoff table.

### State of the project

- Days 1–4 complete. 7 decision docs locked. 4 stale docs flagged for reconciliation before Day 10.
- Days 5–9 remain: 4 tech decisions + 1 sign-off + 6 product/operational decision docs to write.
- Phase 2 coding (Days 10–16) is fully planned in the todo. Windsurf handoff docs are the gate — nothing codes until they're written.

---

---

## Session 4g — Days 5–9 decision sprint + all stale docs fixed (same day, continued)

### What happened

Sprint goal: "decide everything in 2–3 days, fix all redundancies and clashes." All remaining decisions were written and all stale docs reconciled in one session.

**10 decision docs written (parallel blast):**

| Doc | Key lock |
|---|---|
| `voice-stt.md` | Groq Whisper Large v3 Turbo ($20/mo) + Gemini correction pass for medical terms. Upgrade path to Deepgram Nova-3 Medical documented. |
| `ai-routing-fallback.md` | Complexity router triggers locked as-is. 3-attempt fallback for Tier 1, 2-attempt downgrade for Tier 2. |
| `mobile-delivery.md` | Capacitor.js. PWA rejected (discoverability + iOS push). Android immediate, iOS Phase 3. |
| `viva-bot-orchestration.md` | Hand-rolled state machine (ASK → EVALUATE → FOLLOWUP → SCORE). LangGraph rejected. Persisted session. Adaptive difficulty 1–5. |
| `credits-payment.md` | Easypaisa/JazzCash top-up → receipt upload → manual verify 5–10 min. Pro: PKR 3 000/yr debit from credits. Full UX flow. |
| `viva-scoring.md` | 3-mode weights (Strict/Friendly/Standard). 50-point scale. LLM prompt JSON schema. Session report format. Signal injection into student_memory. |
| `dow-id-approval.md` | Admin queue: approve instant / reject with reason picker. Student badge + re-upload CTA. |
| `push-notifications.md` | Ask on Day 2 of usage. Custom priming card. Max 2 lifetime asks. Quiet hours 10 PM–6 AM PKT. |
| `doweats-ops.md` | Item-first menu. 6-digit order code (JetBrains Mono). Gate delivery 11:30–1:30 PM. 15% commission transparent. |
| `marketplace-ops.md` | Listings instant (Realtime). 10% commission. Peer-to-peer handoff. Seller withdrawal PKR 500 min, 0% fees, 2–5 days manual. Manual dispute arbitration. |

**3 stale docs fixed:**
- `5_UXUI_GUIDELINES.md` — IA tree + nav hierarchy updated to locked route tree.
- `00_DISCOVERY_RESOLVED.md` — Maps block updated from Google Maps to MapLibre.
- `FINAL_LOCKED_DECISIONS.md` — Completely rewritten as a clean 17-decision index with resolved conflicts table.

**Conflicts resolved:**
- Rate limits: locked at soft 2 / hard 4 (the 20/50 was a pre-cost-model placeholder).
- Maps: MapLibre GL JS replaces Google Maps SDK in all docs.
- Nav: Dashboard / Education / AI Tutor / Campus / Maps everywhere.

### Files touched

- `docs/decisions/` — 10 new files (see table above)
- `docs/5_UXUI_GUIDELINES.md` — §2 IA tree + §3 nav hierarchy
- `docs/00_DISCOVERY_RESOLVED.md` — maps block
- `docs/FINAL_LOCKED_DECISIONS.md` — full rewrite
- `docs/todo-current.md` — Days 5–9 ticked off, product decisions ticked off, stale docs ticked off

### State of the project

- **Phase 1 complete.** All 17 decisions locked. All conflicts resolved. All stale docs fixed.
- Phase 2 coding (Days 10–16) is fully planned. Windsurf handoff docs are the remaining gate.

---

## Session 4h — Windsurf rules + admin content upload guide (same day, continued)

### What happened

The two remaining Windsurf Handoff docs were written:

**`docs/windsurf-rules.md`** — the always-on context file for Cascade. Covers:
- First-thing instruction: read decision docs before building. Stop-and-flag if no doc exists.
- Full tech stack table (Next.js 15 App Router, TypeScript strict, Tailwind v4, shadcn/ui, Zustand, TanStack Query, Supabase, Capacitor).
- All design tokens inline: colour palette (light + dark), glassmorphism spec, full typography scale.
- Layout & spacing rules (4 px base, 44 px touch targets, breakpoints, NavShell switch at 1024 px).
- Full nav config inline: 5 mobile bottom-nav items + desktop sidebar sections (Main / Study / Campus / Identity / System) with all routes and role gates.
- 13 conventions (skeleton-first, transition timing scale, focus rings, reduced-motion, WCAG AA, next/image, secrets, timezone, currency, Supabase RLS, AI rate limits, branch discipline).
- Stop-and-flag checklist before shipping any component.

**`docs/admin-content-upload.md`** — Azfar's guide. Covers:
- **MCQ CSV:** full column spec (13 columns), example row, validation rules, 6 common mistakes to avoid, upload steps.
- **Textbook PDF:** upload + tag flow (non-technical). Includes "what happens after upload" section: text extraction → splitting → Q&A generation → embedding → indexing.
- **Viva Sheets:** defines the format Azfar should produce (Google Sheet → CSV export). 5-column spec (question, model_answer, difficulty 1–5, key_points, tags). Example rows at 3 difficulty levels. Guidance on target count (10–15 Qs per subtopic) and difficulty distribution.
- Quick-reference table comparing all 3 upload types.
- Target numbers for launch (100 MCQs, 2–3 textbooks, 10–15 Qs × 2–3 subtopics by Days 17–20).

### Files touched

- `docs/windsurf-rules.md` — new
- `docs/admin-content-upload.md` — new
- `docs/todo-current.md` — stale docs ticked off, admin-content-upload ticked off, Windsurf Handoff table updated to all-done

### State of the project

- **Phase 1 fully complete.** All 17 decisions locked. All conflicts resolved. All stale docs fixed. All Windsurf handoff docs written.
- **Ready for Phase 2 coding (Days 10–16).** Next step: copy `windsurf-rules.md` block into `.windsurf/rules`, hand `admin-content-upload.md` to Azfar, then Windsurf starts building NavShell + Dashboard.

---

## What comes next

- **Before Day 10:** Copy the block from `docs/windsurf-rules.md` into `.windsurf/rules`. Hand `docs/admin-content-upload.md` to Azfar.
- **Day 10:** Windsurf builds NavShell (BottomNav + Sidebar + layout switch). Dashboard skeleton. Profile page skeleton.
- **Day 10 (parallel):** Azfar starts producing MCQ CSVs using the format in `admin-content-upload.md`. Target: first 100 MCQs by Day 17.
