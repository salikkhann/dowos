# DowOS — Windsurf Rules

This file becomes `.windsurf/rules`. Cascade reads it on every turn. Keep it under 12 K characters.
Copy the block between the `---` markers into `.windsurf/rules` before the first coding session.
...
── Study ─────────────────
  ...
  Viva Bot           /education/viva       [Pro gate] module→subject→subtopic
  Anki Mode          /education/anki       [Free]     same questions, flashcard drill
