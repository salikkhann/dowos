...
### 3.0 Before the loop — mode picker + greeting

Before the state machine starts, two things happen in order:

**1. Examiner mode picker.** The student picks one of 3 modes. This is a single-tap selection screen, not a toggle inside the session:

| Mode | Personality | Scoring behaviour |
|---|---|---|
| **Friendly** | Encouraging, lenient partial credit, softer feedback | Confidence weighted higher (15 pts). Penalties halved. |
| **Standard** | Balanced, professional | Default weights. |
| **Strict** | Demanding, expects precision, harsh on gaps | Confidence weighted lower (10 pts). Missing key facts = full band drop. R1 used for deep eval. |

The chosen mode is stored in `viva_bot_sessions.mode` and persists for the entire session. It cannot be changed mid-session.

**2. Greeting.** Once the mode is picked and the session is created, the bot delivers an opening greeting **before** asking the first question. The greeting is generated by a single Gemini Flash call with the following context: the student's first name, the chosen subject, and the examiner mode. It sets the tone for the session.

Examples of what the greeting looks like per mode:

| Mode | Example greeting |
|---|---|
| Friendly | "Hi [Name]! Welcome to your Anatomy practice session. Take your time — I'm here to help you learn. Ready when you are!" |
| Standard | "Good afternoon, [Name]. We'll be covering Anatomy today. I'll ask you questions and give you feedback after each answer. Let's begin." |
| Strict | "Good. We're starting with Anatomy. I expect precise, complete answers. No hedging. First question coming up." |

The greeting is spoken aloud via TTS (see `voice-stt.md` §3.5) and displayed as the first message in the session chat. After the greeting, the state machine enters ASK for the first question.

### 3.1 States

```
enum VivaState {
  GREET       = "GREET",        // Opening greeting (fires once, before first ASK)
